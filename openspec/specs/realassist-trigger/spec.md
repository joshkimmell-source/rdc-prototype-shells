# realassist-trigger Specification

## Purpose

Defines where the "Ask RealAssist+" trigger is placed in the agent-web app and how a URL-driven `?ab=` A/B test selects that placement — the available placement variants (`a`, `b`, and `c`), the default/fallback behavior, and the specific location each variant renders the trigger in (a corner floating action button, an inline header action bar, a framed map top bar, or — for the default variant `c` — a responsive combination of these).
...
- **WHEN** the app is opened with an unrecognized `?ab=` value (for example a typo, an empty value, or a value that is not a known variant)
